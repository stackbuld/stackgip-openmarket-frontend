...
@Component({
  selector: 'app-countrycodes-dropdown',
  templateUrl: './countrycodes-dropdown.component.html',
  styleUrls: ['./countrycodes-dropdown.component.scss']
})

export class CountryCodesDropdownComponent implements OnInit {
  ...
  ngOnInit() { 
    this._data
			.subscribe(x => {
        this.selectedBank = x?.find(data => data?.code?.toString() === this.controls?.value?.toString())?.name
      });
    ...
    this._bankData
      .subscribe(value => {
        ...
        this.selectedBank = this.bankLists?.find(data => data?.code?.toString() === value?.bankCode?.toString())?.name
      });
