...
export class GooglePlaceService {
  ...
  httpOptions = {
    headers: new HttpHeaders({
      Origin: this.window.nativeWindow.location.origin,
      "Access-Control-Allow-Origin": this.window.nativeWindow.location.origin,
    }),
  };
  getSearchPlace(place: string): Observable<IPlaceSeachResponse> {
    ...
    const url =
      this.proxyurl +
      `https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${place}&inputtype=textquery&fields=formatted_address,name,geometry&key=AIzaSyDyXDcpICpy-AnVTTwEj6Qiywon3SMqy7w`;
