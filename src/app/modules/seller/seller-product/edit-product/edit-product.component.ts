...
export class EditProductComponent implements OnInit {
  ...
  setProduct({ productId }): void {
    ...
    this.productService.getProductById(productId).subscribe((a) => {
      ...
      this.form.setValue({
        ...
        unit: data.unit,
        paymentOption: [
          //Todo: need to reset payment option from api data
  ...
  flatPaymentOption(
    option: { method: boolean; value: string; label: string }[]
  ) {
    return option
      .filter((opt) => opt.method)
      .map((opt) => opt.value)
      .join(',');
  }
  ...
  getProductUpdatedData(): CreateProductModel {
    return {
      ...
      userId: this.user.id,
      paymentOptions: this.flatPaymentOption(
        this.form.get('paymentOption').value
      ),
      shipments: [],
      options: [],
