...
export class ProductShipmentComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.shipments && changes.shipments.currentValue) {
      ...
      this.setEditForm(changes.shipments.currentValue as ProductShipment[]);
    }
  }
  ...
  updateShipment(shipmentId: number, index: number): void {
    ...
    const shipmentData = (({ state, city, cost }) => ({ state, city, cost }))(
      shipmentControl.value
    ) as ProductShipment;

    this.productService
      .UpdateProductShipment(shipmentId, shipmentData)
      .subscribe((a) => {
  ...
  removeShipment(shipmentId: number, index: number): void {
    ...
    this.productService.deleteProductShipment(shipmentId).subscribe((a) => {
  ...
  addShipment(): void {
    ...
    this.loading = true;
    this.productService
      .createProductShipment(this.productId, data)
      .subscribe((a) => {
