...
export class SellerStoreComponent implements OnInit {
  ...
  createSellerStoreCreate(data) {
    this.dialogService.openDialog(SellerStoreCreateDialogComponent,{data})
    .afterClosed()
    .subscribe((response) => {
  ...
  onDeleteStore(sellerStore) {
    ...
    const message = `By deleting this store, <strong> ${sellerStore.number | 12 } products </strong> in this store will automatically move to the Default Store.`
    const actionButton = `Delete Store <img src="../../../../assets/svg/delete-seller-store-white.svg" alt="" />`
    this.dialogService.openDeleteDialog(title, subtitle, message, actionButton)
    .afterClosed()
    .subscribe((response) => {
  ...
  deleteSeller(sellerStore) {
    this.sellerStoreService.deleteSellerStore(sellerStore)
    .subscribe((res: any) => {
  ...
  getSellerStoreList() {
    this.sellerStoreService.getSellerstores(this.helperService.getLoggedInUserId())
